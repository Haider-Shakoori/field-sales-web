...
final class TenantContext
{
    ...
    public function tenantId(): int
    {
        if (! $this->hasTenant() || $this->tenantId === null) {
            throw new TenantContextMissingException('A tenant-scoped operation was attempted without a tenant context.');
        }
    ...
    public function initializeTenant(Tenant|int $tenant): void
    {
        ...
        if ($this->state === TenantContextState::Platform) {
            throw new TenantContextMismatchException('Refusing to replace an active platform context with a tenant context.');
        }
    ...
    public function initializePlatform(): void
    {
        if ($this->state === TenantContextState::Tenant) {
            throw new TenantContextMismatchException('Refusing to broaden an active tenant context to platform scope.');
        }
    ...
    {
        return $this->temporarily(TenantContextState::Tenant, $tenant instanceof Tenant ? (int) $tenant->getKey() : (int) $tenant, $callback);
    }
    ...
    /**
     * Authentication is the only normal request path that may need a very
     * small platform-scope window before the authenticated user's tenant is
     * known. Once a tenant is already active this method never broadens it.
     */
