...
class SendTrackingReminders extends Command
{
    ...
    public function handle(TrackingSettingsService $settingsService, TenantClock $clock, NotificationService $notifications, TenantContext $context): int
    {
        Tenant::where('subscription_status', 'active')->chunkById(100, function ($tenants) use ($settingsService, $clock, $notifications) {
            foreach ($tenants as $tenant) {
                ...
                if (! $this->insideOrAfterStart($nowLocal, $settings['workday_start_time'], $settings['workday_end_time'])) {
                    continue;
                }

                $start = CarbonImmutable::parse($date.' '.$settings['workday_start_time'], $settings['timezone']);
                ...
                foreach ($salesmen as $salesman) {
                    ...
                    $todayCount = NotificationLog::where('tenant_id', $tenant->id)
                        ->where('recipient_user_id', $salesman->user_id)
                    ...
                    $last = NotificationLog::where('tenant_id', $tenant->id)
                        ->where('recipient_user_id', $salesman->user_id)
                        ->where('type', 'tracking_reminder')
                        ->latest()
                        ->first();
                    ...
                    $session = WorkSession::where('tenant_id', $tenant->id)
                    ...
                    $fresh = CurrentLocation::where('tenant_id', $tenant->id)
    ...
    private function insideOrAfterStart(CarbonImmutable $now, string $start, string $end): bool
    {
