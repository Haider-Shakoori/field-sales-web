...
class StoreCustomerRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        return [
            ...
            'code' => ['nullable', 'string', 'max:50', Rule::unique('customers', 'code')->where('tenant_id', $tenantId)],
    ...
    {
        // If the user is a salesman, auto-assign the customer to them
        $user = $this->user();
        $salesman = $user->salesmanProfile;

        if ($salesman && ! $this->has('assigned_salesman_id')) {
            $this->merge(['assigned_salesman_id' => $salesman->id]);
        }
    }
