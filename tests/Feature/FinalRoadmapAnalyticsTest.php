...
class FinalRoadmapAnalyticsTest extends TestCase
{
    ...
    public function test_reorder_recommendation_and_commission_are_grounded_in_transactions(): void
    {
        ...
        app(TenantContext::class)->withTenant($f['tenant'], function() use($f): void {
            ...
            foreach([now()->subDays(60),now()->subDays(30)] as $i=>$date){$order=Order::create(['user_id'=>$f['salesmanUser']->id,'salesman_id'=>$f['salesman']->id,'customer_id'=>$f['customer']->id,'order_number'=>'REC-'.$i,'ordered_at'=>$date,'payment_type'=>'cash','status'=>'approved','currency'=>'AFN','subtotal'=>1000,'discount_total'=>0,'grand_total'=>1000]); OrderItem::create(['order_id'=>$order->id,'product_id'=>$product->id,'quantity'=>10,'unit_price'=>100,'discount_percent'=>0,'discount_amount'=>0,'line_total'=>1000]);}
