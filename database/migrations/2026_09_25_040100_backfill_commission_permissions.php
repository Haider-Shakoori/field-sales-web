    ...
    public function up(): void
    {
        ...
        foreach ($permissions as $slug => $name) {
            DB::table('permissions')->updateOrInsert(
                ['slug' => $slug],
                ['name' => $name, 'group' => 'commissions', 'updated_at' => $now, 'created_at' => $now],
            );
        }

        $ids = DB::table('permissions')->whereIn('slug', array_keys($permissions))->pluck('id', 'slug');
        $grants = [
            'owner' => ['commissions:view', 'commissions:manage'],
            'company_admin' => ['commissions:view', 'commissions:manage'],
            'sales_manager' => ['commissions:view', 'commissions:manage'],
            'accountant' => ['commissions:view'],
            'auditor' => ['commissions:view'],
        ];
