<x-layouts.app>
    <div class="mb-6 flex flex-wrap items-start justify-between gap-4">
        <div>
            ...
            <p class="mt-1 text-sm text-slate-400">{{ __('Prioritized daily beat plan using route sequence, overdue receivables, follow-ups and visit history.') }}</p>
        </div>
    </div>
    ...
    @elseif(!$plan['route'])
        <div class="rounded-2xl border border-amber-400/20 bg-amber-500/10 p-6">
            <h2 class="font-semibold text-amber-200">{{ __('No route assigned') }}</h2>
            <p class="mt-2 text-sm text-amber-200/80">{{ __('This salesman has no effective route assignment for the selected date.') }}</p>
        </div>
    @else
        <div class="mb-5 rounded-2xl border border-white/10 bg-slate-900 p-5">
            <div class="flex flex-wrap items-center justify-between gap-4">
                <div>
                    <h2 class="font-semibold">{{ $plan['route']['code'] }} — {{ $plan['route']['name'] }}</h2>
        ...
        <div class="mb-5 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
            <div class="rounded-2xl border border-white/10 bg-slate-900 p-5"><p class="text-xs uppercase tracking-wide text-slate-400">{{ __('Remaining stops') }}</p><p class="mt-2 text-2xl font-bold">{{ $plan['summary']['remaining'] }}</p></div>
            <div class="rounded-2xl border border-white/10 bg-slate-900 p-5"><p class="text-xs uppercase tracking-wide text-slate-400">{{ __('Urgent / high') }}</p><p class="mt-2 text-2xl font-bold">{{ $plan['summary']['urgent'] + $plan['summary']['high'] }}</p></div>
            <div class="rounded-2xl border border-white/10 bg-slate-900 p-5"><p class="text-xs uppercase tracking-wide text-slate-400">{{ __('Overdue customers') }}</p><p class="mt-2 text-2xl font-bold">{{ $plan['summary']['customers_with_overdue_balance'] }}</p></div>
            <div class="rounded-2xl border border-white/10 bg-slate-900 p-5"><p class="text-xs uppercase tracking-wide text-slate-400">{{ __('Planned visit time') }}</p><p class="mt-2 text-2xl font-bold">{{ $plan['summary']['planned_visit_minutes'] }} min</p></div>
        </div>
                    ...
                    @foreach($plan['stops'] as $stop)
                        <tr class="{{ $stop['visited_today'] ? 'opacity-60' : '' }}">
                            <td class="px-5 py-4">
                                ...
                                @if($stop['recommended_order'] !== $stop['route_sequence'])
                            ...
                            <td class="px-5 py-4">
                                ...
                                @endphp
                                <span class="rounded-full px-2.5 py-1 text-xs font-semibold {{ $priorityClass }}">{{ __(str($stop['priority'])->title()->toString()) }}</span>
                            </td>
                            ...
                            <td class="px-5 py-4">{{ $stop['last_visited_at'] ? CarbonCarbonImmutable::parse($stop['last_visited_at'])->format('Y-m-d') : __('Never') }}</td>
                            <td class="px-5 py-4">{{ $stop['distance_from_previous_km'] === null ? '—' : number_format($stop['distance_from_previous_km'], 1).' km' }}</td>
                        </tr>
                    @endforeach
                    </tbody>
                </table>
