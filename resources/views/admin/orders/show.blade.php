<x-layouts.app>
    <div class="mb-6 flex flex-wrap items-start justify-between gap-4">
        <div>
            ...
            <p class="mt-1 text-sm text-slate-400">{{ $order->customer?->name }} · {{ __(str($order->status)->title()->toString()) }}</p>
        </div>
    ...
    <div class="grid gap-5 lg:grid-cols-3">
        <section class="rounded-2xl border border-white/10 bg-slate-900 p-5 lg:col-span-2">
            ...
            <div class="mt-5 ml-auto max-w-sm space-y-2 text-sm">
                <div class="flex justify-between"><span class="text-slate-400">{{ __('Subtotal') }}</span><span>{{ number_format((float) $order->subtotal, 2) }} {{ $order->currency }}</span></div>
                <div class="flex justify-between"><span class="text-slate-400">{{ __('Discount') }}</span><span>{{ number_format((float) $order->discount_total, 2) }} {{ $order->currency }}</span></div>
                <div class="flex justify-between border-t border-white/10 pt-2 text-base font-semibold"><span>{{ __('Total') }}</span><span>{{ number_format((float) $order->grand_total, 2) }} {{ $order->currency }}</span></div>
                @if($order->pricing_adjusted)
                    <div class="rounded-xl bg-amber-500/10 p-3 text-amber-200">{{ __('Offline estimate: :amount :currency. Server pricing was applied on sync.', ['amount' => number_format((float) $order->client_estimated_total, 2), 'currency' => $order->currency]) }}</div>
                @endif
            </div>
        </section>
        ...
        <section class="rounded-2xl border border-white/10 bg-slate-900 p-5">
            ...
            <dl class="mt-4 space-y-4">
                ...
                <div><dt class="text-sm text-slate-400">{{ __('Status note') }}</dt><dd>{{ $order->status_note ?? '—' }}</dd></div>
            </dl>
        </section>

        @if(auth()->user()->hasPermission('orders:manage'))
            <section class="rounded-2xl border border-white/10 bg-slate-900 p-5 lg:col-span-3">
                <h2 class="font-semibold">{{ __('Status action') }}</h2>
                ...
                @else
                    <p class="mt-3 text-sm text-slate-400">{{ __('This order is in a terminal status.') }}</p>
                @endif
            </section>
        @endif
    </div>
