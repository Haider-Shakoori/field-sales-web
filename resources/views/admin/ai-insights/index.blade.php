...
(() => {
    ...
    form.addEventListener('submit', async (event) => {
        ...
        try {
            ...
            answerText.textContent = message.content || @json(__('No answer was returned.'));
            answerSource.textContent = message.source === 'configured_ai_agent'
                ? ((message.provider || 'AI').toUpperCase() + (message.model ? ' · ' + message.model : ''))
                : @json(__('Generated locally from FieldPulse metrics without an external AI provider.'));

            if (!activeConversation && payload.conversation?.uuid) {
                const url = new URL(window.location.href);
                url.searchParams.set('chat', payload.conversation.uuid);
