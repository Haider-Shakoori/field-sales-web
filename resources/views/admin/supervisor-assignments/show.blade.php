<x-layouts.app>
    <div class="mb-6 flex flex-wrap items-start justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold">{{ $assignment->supervisor?->full_name }}</h1>
            <p class="mt-1 text-sm text-slate-400">Assignment {{ $assignment->uuid }}</p>
        </div>
        @if(auth()->user()->hasPermission('sales-team:manage'))
            <a href="{{ route('admin.supervisor-assignments.edit', $assignment) }}" class="rounded-xl bg-indigo-500 px-4 py-2.5 font-semibold">Edit</a>
        @endif
    </div>
