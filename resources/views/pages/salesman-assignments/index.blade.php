...
@section('title', 'Salesman Assignments')

@section('content')
    <x-ui.page-header title="Salesman Assignments"
                      description="Manage current and historical salesman assignments.">
        @if (auth()->user()->hasPermission('assignments:manage'))
            <x-slot:actions>
                <x-ui.button href="{{ route('salesman-assignments.create') }}" icon="plus">New assignment</x-ui.button>
            </x-slot:actions>
        @endif
    </x-ui.page-header>

    <form method="GET" action="{{ route('salesman-assignments.index') }}" class="surface-card p-4">
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <x-ui.input name="search" label="Search" :value="request('search')" placeholder="Salesman name or employee code" />
        ...
        <div class="mt-4 flex items-center justify-end gap-2">
            <x-ui.button href="{{ route('salesman-assignments.index') }}" variant="ghost" type="button">Clear</x-ui.button>
    ...
    <x-ui.card>
        @if ($assignments->isEmpty())
            <x-ui.empty-state title="No salesman assignments"
                              icon="clipboard"
                              description="Assignments will appear here once salesmen are allocated to their operational team.">
                @if (auth()->user()->hasPermission('assignments:manage'))
                    <x-slot:action>
                        <x-ui.button href="{{ route('salesman-assignments.create') }}" variant="primary" icon="plus">New assignment</x-ui.button>
                    </x-slot:action>
                @endif
            </x-ui.empty-state>
        @else
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                        @foreach ($assignments as $assignment)
                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/40">
                                <td class="px-3 py-3">
                                    <div class="flex items-center gap-3">
                                        <span class="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-blue-100 text-xs font-semibold text-blue-700 dark:bg-blue-500/20 dark:text-blue-300">
                                            {{ strtoupper(substr($assignment->salesman?->first_name ?? '?', 0, 1)) }}
                                        </span>
                                        <div class="min-w-0">
                                            <p class="font-medium text-gray-900 dark:text-gray-50">
                                                {{ $assignment->salesman ? trim($assignment->salesman->first_name.' '.$assignment->salesman->last_name) : 'Unknown salesman' }}
                                            </p>
                                            <p class="text-xs text-gray-400 dark:text-gray-500">{{ $assignment->salesman?->employee_code ?? '—' }}</p>
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td class="px-3 py-3 text-gray-500 dark:text-gray-400">{{ $assignment->route?->name ?? '—' }}</td>
                                <td class="px-3 py-3 text-gray-500 dark:text-gray-400">
                                ...
                                <td class="px-3 py-3 text-right">
                                    <div class="flex items-center justify-end gap-1">
                                        <a href="{{ route('salesman-assignments.show', $assignment) }}"
                                        ...
                                        @can('update', $assignment)
                                            <a href="{{ route('salesman-assignments.edit', $assignment) }}"
