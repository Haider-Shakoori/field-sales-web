...
@section('content')
    ...
    <x-ui.page-header title="Supervisor assignment"
        ...
        <x-slot:actions>
            @can('update', $assignment)
                <x-ui.button href="{{ route('supervisor-assignments.edit', $assignment) }}" icon="pencil">Edit</x-ui.button>
            @endcan
            <x-ui.button href="{{ route('supervisor-assignments.index') }}" variant="ghost" icon="clipboard">All assignments</x-ui.button>
        </x-slot:actions>
    </x-ui.page-header>
        ...
        <div class="space-y-6 xl:col-span-2">
            <x-ui.card title="Assignment details">
                <dl class="grid grid-cols-1 gap-x-4 gap-y-3 sm:grid-cols-2">
                    ...
                    <dd class="text-sm font-mono text-gray-900 dark:text-gray-50">{{ $assignment->supervisor?->employee_code ?? '—' }}</dd>
            ...
            @can('deactivate', $assignment)
                <x-ui.card title="Danger zone">
                    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                        ...
                        <form method="POST"
                              action="{{ route('supervisor-assignments.destroy', $assignment) }}"
                              onsubmit="return confirm('Delete this supervisor assignment? This cannot be undone.')">
                            @csrf
                            @method('DELETE')
